...
const Dictionary = () => {
    ...
    //  const unsplashurl=`https://api.unsplash.com//photos?page=1&query=${inword}&client_id=p7Sl3eBqlIgMOZnRAGaC0g0ACR3lf5risDUIPGZDeXc` 
...
<div className='flex  items-center flex-col'>
      ...
      { !loading&&  <div className='flex flex-col w-full '>
            ...
            <div className='lg:w-[500px] md:w-[300px] w-[250px] sm:w-[300px] h-[140px]  md:h-[150px] sm:h-[150px] lg:h-[250px] mx-auto rounded-md border border-b-white'>
   

       <img src={image} className='object-cover w-full h-full aspect-auto'/>
               </div>
            <div className='flex gap-x-[30px] w-full p-[10px]'>
                  <p className=' font-bold text-brown-200 text-3xl'>{inword}</p>
                  <button onClick={clickaud}><AiFillSound className='text-blue-100 text-2xl'/></button>
            </div>
       ...
       <div className='w-full h-[500px]  text-white opacity-80 text-3xl flex items-center justify-center'>
        ...
        {/* <img src={dicti} className='absolute left-[100%]  top-0'/> */}
        
     </div>
    </div>
    //   <div className='flex flex-col border border-white h-[200px]  overflow-scroll'>
    //       {
    //             syn.map((data,index)=>{
    //                 return <p className='text-white ' key={index}>{data}</p>
    //             })
    //         }
