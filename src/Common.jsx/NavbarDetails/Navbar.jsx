...
setShow(true)
  ...
  const{setData1}=useContext(Appcontext)

function clickdrop(e){
    e.preventDefault()
    document.querySelector("dropboxx").add("myclss")
}
function donecl(){
    setClick(!click)
...
{show&&<div className='  hidden gap-x-7  lg:flex   justify-center   items-center'>
     ...
     <Link to="/AboutUs" className='text-white font-bold'>AboutUs</Link>
...
<div className='flex flex-col gap-y-3 border-yellow-5 '>
     ...
     {/* <Link to="/AboutUs" className={`text-white font-bold w-full ${matchRoute('/AboutUs') ? "bg-yellow-800 border-l-yellow-100 border-l-[2px] text-yellow-100" :"bg-opacity-0"}     `}><div className='flex justify-between py-2  w-[70%] mx-auto opacity-90 '><CgGames className='text-2xl'/>Games</div></Link> */}
