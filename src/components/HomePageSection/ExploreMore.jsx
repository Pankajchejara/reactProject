...
const ExploreMore = () => {
  ...
  const setMyCards = (value) => {
    ...
    console.log("bhai result kya aaya hai",result)
  ...
  return (
    <div className=" w-full h-full  flex flex-col gap-y-6 py-7">
      <div className="flex flex-col gap-y-2 lg:flex-row border w-[200px] border-blue-300 lg:border-none items-center justify-center gap-x-4 lg:w-11/12 mx-auto   rounded-xl">
        {
            tabsName.map((data)=>{
                return <p className="text-white hover:text-yellow-5 cursor-pointer border-cyan-400 rounded-md border py-1 px-3" onClick={()=>setMyCards(data)}>{data}</p>
            })
        }
     ...
     <div className="w-11/12 flex justify-center items-center gap-4 flex-wrap">
        {
            courses.map((data)=>{
                ...
                return<Tilt>
                    <GlowCapture>
                        <Glow>
                 <div className="w-[300px] h-[200px] flex flex-col gap-y-4 shadow-md  glow:bg-cyan-100/20 cursor-pointer shadow-cyan-400">
